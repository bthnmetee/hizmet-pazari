        ...
        <div className="bg-white border border-navy-100 rounded-2xl overflow-hidden shadow-sm">
            ...
            >
                <div className="flex items-center gap-3">
                    <div className="w-9 h-9 bg-navy-800 rounded-xl flex items-center justify-center text-white text-sm font-black">
                        {currentIndex + 1}/{SIRALAMA.length}
                    </div>
                    <div className="text-left">
                        <p className="text-sm font-black text-navy-900">İş Durumu</p>
                        {profesyonelAdi && <p className="text-xs text-navy-300">{profesyonelAdi} ile</p>}
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-navy-700 bg-navy-50 px-2 py-1 rounded-full">
                        {AŞAMALAR.find(a => a.id === currentStatus)?.label}
                    </span>
                    <span className={`text-navy-300 transition-transform ${expanded ? 'rotate-180' : ''}`}>⌄</span>
                </div>
            </button>

            {/* İlerleme çubuğu */}
            <div className="mx-5 h-1.5 bg-navy-100 rounded-full overflow-hidden">
                <div
            ...
            {expanded && (
                <div className="px-5 py-5">
                    <div className="relative">
                        {/* Dikey çizgi */}
                        <div className="absolute left-[22px] top-6 bottom-6 w-px bg-navy-100" />
                      ...
                      ${done ? 'bg-navy-50 shadow-emerald-100' : ''}
                      ${active ? 'bg-navy-800 shadow-emerald-200 shadow-lg scale-110' : ''}
                      ${upcoming ? 'bg-navy-50/50' : ''}
                    `}>
                                            {done ? (
                                                <span className="text-gold-500 font-black text-base">✓</span>
                                            ) : (
                                        ...
                                        <div className="flex-1 min-w-0 pt-1">
                                            <div className="flex justify-between items-start">
                                                <p className={`text-sm font-black ${active ? 'text-navy-700' : done ? 'text-navy-700' : 'text-navy-300'}`}>
                                                    {aşama.label}
                                                </p>
                                                {tarih && (
                                                    <span className="text-xs text-navy-300 font-medium shrink-0 ml-2">{formatTarih(tarih)}</span>
                                                )}
                                            </div>
                                            <p className={`text-xs font-medium mt-0.5 ${active ? 'text-navy-600' : 'text-navy-300'}`}>
                                                {aşama.desc}
                                            </p>
                                            {active && aşama.actionLabel && onAction && (
                                                <button
                                                    onClick={() => onAction(aşama.actionId!)}
                                                    className="mt-2 text-xs font-black text-white bg-navy-800 hover:bg-navy-800 px-3 py-1.5 rounded-xl transition-all hover:-translate-y-0.5 shadow-sm shadow-emerald-200"
                                                >
                                                    {aşama.actionLabel} →
                                                </button>
                                            )}
                    ...
                    {currentStatus !== 'degerlendirildi' && (
                        <div className="mt-4 pt-4 border-t border-gray-50 flex items-center gap-2 text-xs text-navy-300 font-medium">
                            <span>⏱</span>
                            <span>
                                Tahmini tamamlanma: <strong className="text-navy-600">
